...
public class puntaje {
    ...
    private static final String SCORE_PREF = "Puntaje preferences";
    private Bitmap zero;
    private Bitmap one;
    private Bitmap two;
    ...
    public puntaje(Resources resources, int screenHeight, int screenWidth) {
    ...
    public void draw(Canvas canvas) {
        ...
        if(!collision) {
            ...
            for (int i = 0; i < digits.size(); i++) {
                int x = screenWidth / 4 - digits.size() * zero.getWidth() + zero.getWidth() * i;
                int y = 3 * screenWidth / 4 -digits.size() * zero.getWidth() + zero.getWidth() * i;
                canvas.drawBitmap(digits.get(i), x, 3 * screenHeight / 4, null);
                canvas.drawBitmap(topDigits.get(i), y, 3 * screenHeight / 4, null);
            }
